...
const JitoTipAccount = 'ADuUkR4vqLUMWXxW9gh6D6L8pMSawimctcNZ5pGwDcEt';
...
let openBookSubscriptionId: number;
let walletSubscriptionId: number;
let wallet: Keypair;
...
async function buy(accountId: PublicKey, accountData: LiquidityStateV4): Promise<void> {
  try {
    console.log(accountId.toString(), 'account Id');
      ...
      logger.info(
        {
          ...
          url: `https://solscan.io/tx/${response.data.result}?cluster=${NETWORK}`,
        },
        `Confirmed buy tx`,
      );
    } else {
      logger.debug(response.data);
      logger.info({ mint: accountData.baseMint, signature: response.data.result }, `Error confirming buy tx`);
    }
  } catch (e) {
    logger.debug(e);
...
function shouldBuy(key: string): boolean {
  logger.info(`-------------------------------------- `);
  logger.info(`Processing token: ${key.toString()}`);
  logger.info(
    `Correct token: ${USE_SNIPE_LIST ? snipeList.includes(key) : ONE_TOKEN_AT_A_TIME ? !processingToken : true}`,
  );
...
export const runListener = async () => {
  ...
  raydiumSubscriptionId = solanaConnection.onProgramAccountChange(
    ...
    async (updatedAccountInfo) => {
      ...
      if (poolOpenTime > runTimestamp && !existing) {
        ...
        console.log(updatedAccountInfo.accountId, poolState, '====================');
  ...
  openBookSubscriptionId = solanaConnection.onProgramAccountChange(
    OPENBOOK_PROGRAM_ID,
    async (updatedAccountInfo) => {
      const key = updatedAccountInfo.accountId.toString();
  ...
  if (AUTO_SELL) {
    walletSubscriptionId = solanaConnection.onProgramAccountChange(
      TOKEN_PROGRAM_ID,
      async (updatedAccountInfo) => {
  ...
  if (USE_SNIPE_LIST) {
    interval = setInterval(loadSnipeList, SNIPE_LIST_REFRESH_INTERVAL);
  }
};

export const exitBot = () => {
  clearInterval(interval);
  solanaConnection.removeProgramAccountChangeListener(raydiumSubscriptionId);
  solanaConnection.removeProgramAccountChangeListener(openBookSubscriptionId);
  if (walletSubscriptionId)
    solanaConnection.removeProgramAccountChangeListener(walletSubscriptionId);
}
